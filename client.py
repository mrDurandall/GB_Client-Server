...
from client.connect_window import ConnectWindow
from client.client_DB import ClientDatabase
from client.network_interface import Client
...
def main():
    ...
    if not server_port or not host_number or not username:
        connect_app = QApplication(sys.argv)
        connect_window = ConnectWindow()
        connect_window.show()
        connect_app.exec()

        if connect_window.ok_pressed:
            del connect_window
        else:
            exit(0)
        ...
        config.read(f'{dir_path}/client/client.ini')

        server_port = int(config['SETTINGS']['port'])
        host_number = config['SETTINGS']['server_ip']
